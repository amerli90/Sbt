...
ClassImp(SbtCorrelatedMultipleScattering)

SbtCorrelatedMultipleScattering::SbtCorrelatedMultipleScattering(SbtGenAlg* genAlg) :
  SbtMultipleScattering(genAlg) {
  if (getDebugLevel() > 0) {
    std::cout << "SbtCorrelatedMultipleScattering:  DebugLevel= " << getDebugLevel() << std::endl;
  }
}

void SbtCorrelatedMultipleScattering::MultipleScattering(TVector3& point, TVector3& direction, double RadLen) {
  ...
  if (RadLen == 0) {
    if (getDebugLevel() > 3) std::cout << "Radiation lenght is 0! No MS effect will be simulated" << std::endl;
  }
  else {
    ...
    if (Sigma == 0.) {
      ...
      TVector3 CorrP = direction_p * dist * thetaP2;
      TVector3 CorrQ = direction_q * dist * thetaQ2;
